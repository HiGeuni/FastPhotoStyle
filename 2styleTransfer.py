...
from extractLabel import extractImage
...
def getEquationTwo(content_image, style_image):
    contnetDc = extractImage(content_image, True)
    styleDc = extractImage(style_image, True)
    labels = contentDc.keys()
    deno = 0
    mole = 0
    for label in labels:
        if float(contentDc[label]) == 0.0:
            deno += float(styleDc[label])
        elif float(styleDc[label]) == 0.0:
            deno += float(contentDc[label])
        else:
            deno += max(float(contentDc[label]), float(styleDc[label]))
            mole += min(float(contentDc[label]), float(styleDc[label]))
    eq1 = float(mole/deno)
...
def getEquation(content_image, target):
    ...
    comp = 0
    resv = ''
    contentDc = extractImage(content_image, True)
    print(222222222222222)
    contentLabels = list(contentDc.keys())
    equation = [0.0, 0.0]
    for t in range(target.shape[0]):
        ...
        deno = 0
        mole = 0
        for label in contentLables:
            d = target.loc[t, label]
            if float(contentDc[label]) == 0.0:
                deno += float(d)
            elif float(d) == 0.0:
                deno += float(contentDc[label])
            else:
                deno += max(float(contentDc[label]), float(d))
                mole += min(float(contentDc[label]), float(d))
        eq1 = float(mole/deno)
        content = cv2.imread(content_image)
        style = cv2.imread(target.loc[t, 'file_name'])
        if content.shape[0]*contentshape[1] > style.shape[0] * style.shape[1]:
            style = cv2.resize(style, dsize=(content.shape[1], content.shape[0]), intepolation=cv2.INTER_CUBIC)
        else:
            content = cv2.resize(content, dsize = (style.shape[1], style.shape[0]), interpolation=cv2.INTER_CUBIC)

        grayContent = cv2.cvtColor(content, cv2.COLOR_BGR2GRAY)
        grayStyle = cv2.cvtStyle(style, cv2.COLOR_BGR2GRAY)
        (score, diff) = ssim(grayContent, grayStyle, full=True)
        eq2 = score
